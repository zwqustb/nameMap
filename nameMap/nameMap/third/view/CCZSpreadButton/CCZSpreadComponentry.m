...
CGFloat        const CCZSpreadDis = 100; //默认弹出距离
CGFloat        const CCZAutoFitRadiousSpace = 0; // 自动适应items之间的弧度空隙
CGFloat        const CCZRadiusStep = 5; // 递归变量递增值
#define kCCZSCREEN_BOUNDS self.rtPopBounds//[[UIScreen mainScreen] bounds]
...
- (void)_spreadBasicSetting {
    ...
    _offsetAngle = 0;
    _canClickTempOn = YES;
    _autoAdjustToFitSubItemsPosition = YES;
}
...
- (void)panToSpread:(UIPanGestureRecognizer *)pan {
    ...
    if (pan.state == UIGestureRecognizerStateEnded || pan.state == UIGestureRecognizerStateCancelled || pan.state == UIGestureRecognizerStateFailed) {
        if (!_spreadButtonOpenViscousity) {
            CGPoint ptLoc=self.frame.origin;
            [self panEnd:ptLoc];
            return;
        }
...
- (BOOL)calInitialAngleWithTotalAngle:(CGFloat *)angle offsetAngle:(CGFloat *)sAngle {
    ...
    CGPoint cp = CGPointMake(self.frame.origin.x + self.frame.size.width / 2-_ptScrollOffset.x, self.frame.origin.y + self.frame.size.height / 2-_ptScrollOffset.y);
    // a1 item1偏移y轴的弧度
    // a2 itemn偏移x轴的弧度
    // at 偏移的角度
    // ac 多余的弧度
    ...
    //zwqbgn
//    CGFloat fBoundsH=kCCZSCREEN_BOUNDS.size.height;
//    CGFloat dltH=fBoundsH-lmax;
//    CGFloat dltH1=dltH - cp.y  + l;
//    BOOL bCanAcos=(ABS(dltH1/l)<=1);
    //zwqend
